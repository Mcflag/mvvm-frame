...
import cn.lcsw.diningpos.databinding.ActivityTradeBeginTimeBinding
...
import kotlinx.android.synthetic.main.activity_trade_begin_time.*
...
class BeginTimeActivity : BaseActivity<ActivityTradeBeginTimeBinding>() {

    override val kodein: Kodein = Kodein.lazy {
        extend(parentKodein)
    }

    override val layoutId: Int = R.layout.activity_trade_begin_time
    ...
    override fun initView() {
        ...
        val s2 = SpannableString("按【确认】键默认为今日零点")
        s2.setSpan(
        ...
        scanner.setOnScanSuccessListener(object : DefaultScanSuccessListener() {
            override fun onScanSuccess(barcode: String) {
                if (ClickUtil.isFastClick()) {
                    ...
                    prefs.beginTime = SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Timestamp(zero))
                    EndTimeActivity.launch(this@BeginTimeActivity)
                }
            }
    ...
    private fun setTime(barcode: String) {
        ...
        if (barcode.length == 14 && tv_second.text.isNotEmpty()) {
            if (check(barcode)) {
                prefs.beginTime = convert(barcode)
                EndTimeActivity.launch(this)
            } else {
    ...
    companion object {
        fun launch(activity: AppCompatActivity) =
            activity.apply {
                val intent = Intent(this, BeginTimeActivity::class.java)
    ...
    private fun check(time: String): Boolean {
        ...
        var gap: Long = onDay * 31 * 3
        if (inputTime > current){
            message = "输入时间错误，开始时间不能大于当前时间"
        }
        if((current - inputTime) > gap){
            message = "输入时间错误，只能查询3个月内的数据"
        }
        return inputTime < current && (current - inputTime) < gap
    }
